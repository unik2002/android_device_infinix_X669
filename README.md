# Device Tree — Infinix HOT 30i (X669)

## Device Specifications

| Feature | Specification |
|---|---|
| **Device** | Infinix HOT 30i |
| **Codename** | X669 |
| **SoC** | Unisoc T606 (UMS9230) |
| **Architecture** | ARM64 (arm64-v8a) |
| **RAM** | 4GB |
| **Storage** | 128GB (F2FS) |
| **Display** | 720 x 1612, 400 DPI |
| **Android** | 12 (VNDK 31) |
| **Kernel** | 5.4.254 GKI (android12) |
| **Boot** | Virtual A/B, Header v4 |

## Build Status
> Work in progress — not yet building

## Sources
- Device tree: https://github.com/YourUsername/android_device_infinix_X669
- TWRP reference: https://github.com/Massatriof16/X669

## Notes
- Super partition: 5,872,025,600 bytes (~5.47GB)
- No recovery partition (vendor_boot ramdisk)
- GKI kernel — prebuilt DTB required
